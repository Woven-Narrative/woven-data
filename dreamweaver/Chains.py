...
def find_similar_projects(criteria: str, filters: dict) -> pd.DataFrame:
    """
    ...
    user_embedding = _model.embed_request(query=criteria)
    
    # request similar documents with filtering
    similar_documents_df = _db.get_similar_documents(match_threshold=0.75, 
                                                            match_count=db_match_count, 
                                                            text_embedding=str(user_embedding))        
    
    # get the llm object
    open_ai_llm = _model.get_llm()
    ...
    for t in similar_documents_df["text"].tolist():
        ...
        document_summaries.append(open_ai_llm(prompt_1))                        
